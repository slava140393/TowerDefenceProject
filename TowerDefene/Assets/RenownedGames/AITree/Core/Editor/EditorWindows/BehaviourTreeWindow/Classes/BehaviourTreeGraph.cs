    ...
    public class BehaviourTreeGraph : GraphView
    {
        ...
        public override void BuildContextualMenu(ContextualMenuPopulateEvent evt)
        {
            ...
            else if (evt.target is WrapView nodeView)
            {
                ...
                {
                    DeleteElements(new List<NodeView>() { nodeView });
                    if (nodeView.GetInputPort() != null)
                    {
                        DeleteElements(nodeView.InConnections);
                    }
                    if (nodeView.GetOutputPort() != null)
                    {
                        DeleteElements(nodeView.OutConnections);
                    }
                });
            }
            else if (evt.target is DecoratorView decoratorView)
            {
                evt.menu.AppendAction("Delete", a => DeleteElements(new List<DecoratorView>() { decoratorView }));
            }
            else if (evt.target is ServiceView serviceView)
            {
                evt.menu.AppendAction("Delete", a => DeleteElements(new List<ServiceView>() { serviceView }));
            }
        }
        ...
        private void UpdateNodesSequenceNumber()
        {
            ...
            BehaviourTree.Traverse(behaviourTree.GetRootNode(), n =>
            {
                if (n is RootNode) return;
        ...
        private void OnDoubleClickSelection(VisualElement target)
        {
            ...
            if (nodeView != null)
            {
                ...
                if (node is CompositeNode || node is RootNode)
                {
                    ...
                    BehaviourTree.Traverse(node, n =>
                    {
                        AddToSelection(FindNodeView(n) as WrapView);
                    });
                }
